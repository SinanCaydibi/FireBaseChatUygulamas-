...
public class RegisterActivity extends AppCompatActivity {
    ...
    public void  init(){
        ...
        txtPassword=(EditText) findViewById(R.id.txtPaswordRegister);
    ...
    private void YeniKullanici() {
        ...
        if(TextUtils.isEmpty(email)){
            ...
            Toast.makeText(this,"Password Alanı BOş olamaz !",Toast.LENGTH_LONG).show();

        }else{
            //Sistem tarafından kontrol ediliyor .
            auth.createUserWithEmailAndPassword(email,password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                @Override
                public void onComplete(@NonNull Task<AuthResult> task) {
                    if(task.isSuccessful()){
                        Toast.makeText(RegisterActivity.this,"Password Alanı BOş olamaz !",Toast.LENGTH_LONG).show();
                        Intent loginIntent =new Intent(RegisterActivity.this,LoginActivity.class);
                        startActivity(loginIntent);
                        finish();
                    }else{
